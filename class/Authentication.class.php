...
class Authentication
{
	...
	public function encodingVideoSelect ( $token = '' , $filesKey , $type )
	{
		...
		{
			if ( $re == strip_tags ( $re ) )
				return json_decode($re)->Message ;
			return $re ;
		}

		return $this -> returnMsg ( $re , __FUNCTION__ , $filesKey ) ;
	}
	...
	public function encodingFileDefaultplayUpdate ( $token = '' , $fileKey , $encodeKey , $type )
	{
		...
		return $this -> returnMsg ( $re , __FUNCTION__ , $fileKey ) ;
	}
	 ...
	 * 인코딩 파일 삭제 요청
	 * @param string $token 인증토큰
	 * @param string $encodingKeys 인코딩 파일 키
	 * @return array 삭제 결과 ( RequestID : 요청번호 ; Result : 결과 메시지 )
	 */
	public function encodingFilesDelete ( $token = '' , $encodingKeys )
	{
		$_token = $token ? $token : ($this -> token ? $this -> token : NULL ) ;
		if ( ! $_token )
			return 'No token' ;
		...
		$headers[] = 'Authorization:' . $_token ;
		$re = self::curl ( self::$encodingFilesUrl . json_encode ( $encodingKeys ) , $headers , 'DELETE' ) ;
		return $this -> returnMsg ( $re , __FUNCTION__ , $encodingKeys ) ; // $re -> url
	}
	 ...
	 * 워터마크 삭제
	 * @param string $token 인증토큰
	 * @param array $watermarks 삭제할 워터마크 이름
	 * @return array 워터마크 ( RequestID : 요청번호 ; Result : 결과 메시지 )
	 */
	public function watermarkDelete ( $token = '' , $watermarks )
	{
		$_token = $token ? $token : ( $this -> token ? $this -> token : NULL ) ;
		if ( ! $_token )
			return 'No token' ;
		...
		$headers[] = 'Authorization:' . $_token ;
		$re = self::curl ( self::$watermarkUrl . '&watermarks=' . json_encode ( $watermarks )  , $headers , 'DELETE' ) ;
		return $this -> returnMsg ( $re , __FUNCTION__ ) ;
	}
}
